...
__INTERNAL_tcf_do_hack() {
   ...
   if [[ -z "$TCF_NOHACK" ]]; then
    tcfChk "Apply TCF beakerlib hacks" && {
      ...
      echo -n "patching __INTERNAL_LogAndJournalFail"
      ...
      if declare -F __INTERNAL_WriteToMetafile &> /dev/null; then
        echo -n ", __INTERNAL_LogText"
        local rlL=$(declare -f __INTERNAL_LogText | sed -r 's|(MESSAGE(_COLORED)?="\$prefix(_colored)? )(\$MESSAGE(_COLORED)?)|\1${__INTERNAL_Log_prefix}\4|')
        eval "$rlL"
        echo -n ", rljAddTest"
...
tcfTry() {
  ...
  if [[ $__INTERNAL_tcf_result -eq 0 ]]; then
    ...
    declare -p __INTERNAL_tcf_current_level_data | LogMoreHigh -f "internal data='$(cat -)'"
    pp="BEGIN"
    incr=1
  fi
  if [[ -n "$title" ]]; then
...
tcfRun() {
  ...
  #optsDone; eval "${optsCode}"
    local GETOPT=$(getopt -o .f:n -l fail-tag:,timeout::,kill-timeout:,kt:,signal:,check-code:,kill-code:,allow-skip,as,no-assert -- "$@")
    [[ $? -ne 0 ]] && {
      echo 'Exiting'
      return 1 >& /dev/null
      exit 1
    ...
    while [[ -n "$1" ]]; do
      ...
      --)
        ...
        ;;
      --fail-tag|-f)
        optsPresent="${optsPresent}failtag "
        shift
        failtag+=( "$1" )
        ;;
      --timeout)
        optsPresent="${optsPresent}timeout "
        shift
        timeout="${1:-10}"
        ;;
      --kill-timeout|--kt)
        optsPresent="${optsPresent}killtimeout "
        shift
        killtimeout+=( "$1" )
        ;;
      --signal)
        optsPresent="${optsPresent}signal "
        shift
        signal+=( "$1" )
        ;;
      --check-code)
        optsPresent="${optsPresent}checkcode "
        shift
        checkcode+=( "$1" )
        ;;
      --kill-code)
        optsPresent="${optsPresent}killcode "
        shift
        killcode+=( "$1" )
        ;;
      --allow-skip|--as)
        optsPresent="${optsPresent}allowskip "
        allowskip+=( '1' )
        ;;
      --no-assert|-n)
        optsPresent="${optsPresent}noassert "
        noassert+=( '1' )
        ;;
      *)
        echo "unknown option '$1'"
    ...
    local tmp
    tmp="\<failtag\>";     [[ "$optsPresent" =~ $tmp ]] || local failtag=''
    tmp="\<timeout\>";     [[ "$optsPresent" =~ $tmp ]] || local timeout='10'
    tmp="\<killtimeout\>"; [[ "$optsPresent" =~ $tmp ]] || local killtimeout='5'
    tmp="\<signal\>";      [[ "$optsPresent" =~ $tmp ]] || local signal='TERM'
    tmp="\<checkcode\>";   [[ "$optsPresent" =~ $tmp ]] || local checkcode='kill -0 $cmdpid >&/dev/null'
    tmp="\<killcode\>";    [[ "$optsPresent" =~ $tmp ]] || local killcode='/bin/kill -$signal -- $cmdpid'
    tmp="\<allowskip\>";   [[ "$optsPresent" =~ $tmp ]] || local allowskip=''
    tmp="\<noassert\>";    [[ "$optsPresent" =~ $tmp ]] || local noassert=''
  ...
  ###############
  #local t2=$(date +"%s%N")
  #rlLogInfo "$FUNCNAME(): opts parsing took $(( t2-t1 )) nanoseconds"
